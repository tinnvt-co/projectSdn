...
function RDetailModal({ report, onClose }) {
    const color = R_TYPE_COLORS[report.type] || "#6366f1";
    const isPending = report.status === "pending";

    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-box wide" onClick={(event) => event.stopPropagation()}>
                ...
                <div className="detail-grid">
                    <div className="detail-item">
                        ...
                        <span className="d-val">
                            <span style={{ background: `${color}20`, color, padding: "3px 10px", borderRadius: 20, fontSize: 13 }}>
                                {R_TYPE_LABELS[report.type]}
                            </span>
                        </span>
                    </div>
                    <div className="detail-item">
                        <span className="d-label">Trạng thái</span>
                        <span className={`d-badge ${isPending ? "pending" : "reviewed"}`}>{isPending ? "Chờ duyệt" : "Đã duyệt"}</span>
                    </div>
